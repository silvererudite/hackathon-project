...
# ------------------------------------------------------------------------
# Code-agent backend surface (from feature/generative).
# ------------------------------------------------------------------------
# code_agent.py drives a DIFFERENT agent design: rather than choosing from a
# fixed tool set, the model writes Python that code_runner.py executes in a
# subprocess. That needs a plain JSON-in/JSON-out call, not the tool-calling
# loop above -- which is what chat_json provides. Both designs emit the same
# Trajectory schema, so their traces compare directly. Keeping both is the
# point: tool-calling vs code-writing on an identical task.
